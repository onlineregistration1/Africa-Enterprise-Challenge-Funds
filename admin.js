...
const ADMIN_USER="Kibet Davis"; const ADMIN_PASS="Kwenik254$";
const $=id=>document.getElementById(id);
let activePhone=null; let unsub=null;

if(localStorage.getItem('admin')==='1'){ showAdmin(); }

$('aLogin').onclick=()=>{ if($('aUser').value===ADMIN_USER && $('aPass').value===ADMIN_PASS){ localStorage.setItem('admin','1'); showAdmin(); }else $('aMsg').textContent='Wrong credentials'; }
$('aLogout').onclick=()=>{localStorage.removeItem('admin');location.reload();}
$('backBtn').onclick=()=>{closeChat();}

function showAdmin(){ $('loginScreen').classList.add('hidden'); $('adminScreen').classList.remove('hidden'); loadUsers(); }
...
async function sendMsg(text='',imgUrl=''){
  await addDoc(collection(db,'chats',activePhone,'messages'),{ text, img:imgUrl||'', sender:'admin', ts:serverTimestamp(), delUser:false, delAdmin:false });
